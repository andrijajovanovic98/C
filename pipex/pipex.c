...
/*   Updated: 2024/06/17 09:30:47 by ajovanov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "pipex.h"
...
void	initialize_mystr(int ac, char **av, t_struct *mystr)
{
	...
	mystr->infile = open(av[1], O_RDONLY);
	if (mystr->infile < 0)
		error();
	mystr->outfile = open(av[ac - 1], O_TRUNC | O_CREAT | O_RDWR, 0644);
	if (mystr->outfile < 0)
		error();
...
int	main(int ac, char **av, char **envp)
{
	...
	if (mystr.cmd_paths == NULL)
		return (free(mystr.paths), 0);
	mystr.pid1 = fork();
	if (mystr.pid1 == 0)
		first_child(mystr, av, envp);
	mystr.pid2 = fork();
	if (mystr.pid2 == 0)
		second_child(mystr, av, envp);
	close_pipes(&mystr);
